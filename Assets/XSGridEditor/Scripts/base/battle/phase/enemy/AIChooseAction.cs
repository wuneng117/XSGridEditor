...
namespace XSSLG
{
    ...
    public class AIChooseAction : PhaseBase
    {
        public override void OnEnter<T>(T logic)
        {
            ...
            XSU.CameraMoveTo(logic.UnitMgr.ActionUnit.WorldPos);
        }
        ...
        {
            base.OnExit(logic);
            var gridShowMgr = XSU.GetBattleNode().GridShowMgr;
        ...
        private bool CanAttack<T>(T logic) where T : BattleLogic
        {
            ...
            var AttackRegion = XSU.GetBattleNode().GridShowMgr.ShowAttackRegion(logic.UnitMgr.ActionUnit, skill);
            ...
            XSU.GetBattleNode().GridShowMgr.ClearAttackRegion();
            return false;
        }
    }
}