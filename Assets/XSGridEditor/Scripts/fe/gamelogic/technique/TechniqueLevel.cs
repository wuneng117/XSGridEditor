...
namespace XSSLG
{
    ...
    public class TechniqueLevelEx
    {
        ...
        public int GetLv(XSDefine.TechniqueType type) => type >= XSDefine.TechniqueType.Max ? 0 : this.TechniqueArray[(int)type].Lv;
        
        /// <summary> 增加经验值并看看能否升级 </summary>
        public bool AddExpAndCheckLevel(XSDefine.TechniqueType type, int addExp) => type >= XSDefine.TechniqueType.Max ? false : this.TechniqueArray[(int)type].AddExpAndCheckLevel(addExp);
        public TechniqueLevelEx(List<TechniqueLevel> dataArray)
        {
            // 生成初值，因为this.TechniqueArray[2]没有赋值这样调用会报错。。。
            int max = (int)XSDefine.TechniqueType.Max - 1;
