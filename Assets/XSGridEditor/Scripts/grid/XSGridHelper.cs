...
namespace XSSLG
{
    ...
    public class XSGridHelper : MonoBehaviour
    {
        ...
        /// <summary> prefab for attack range </summary>
        [SerializeField]
        private GameObject attackTilePrefab;
        public GameObject AttackTilePrefab { get => attackTilePrefab; }

        /// <summary> prefab for attack effect range </summary>
        [SerializeField]
        private GameObject attackEffectTilePrefab;
        public GameObject AttackEffectTilePrefab { get => attackEffectTilePrefab; }
        ...
        public virtual bool SetTransToTopTerrain(Transform objTransform, bool closeUnit)
        {
            ...
            if (closeUnit)
            {
                XSU.ActionChildren(this.unitRoot?.gameObject, (child) => child.SetActive(false));
            }
            ...
            if (closeUnit)
            {
                XSU.ActionChildren(this.unitRoot?.gameObject, (child) => child.SetActive(true));
            }
            return ret;
        }
    }
