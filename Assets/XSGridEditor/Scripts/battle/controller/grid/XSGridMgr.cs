...
namespace XSSLG
{
    ...
    public class XSGridMgr : XSIGridMgr
    {
        ...
        public virtual void Init(XSGridHelper helper)
        {
            ...
            foreach (var pair in this.TileDict)
            {
                foreach (var pos in NearPosArray)
                {
                    ...
                    if (this.TryGetXSTile(nearPos, out var tile) &&
        ...
        {
            if (this.HasXSTileByWorldPos(tileNode.WorldPos))
            {
                this.RemoveXSTileByWorldPos(tileNode.WorldPos);
            }
        ...
        {
            var tilePos = this.WorldToTile(worldPos);
            if (this.TryGetXSTile(tilePos, out var tile))
            {
            ...
            {
                Debug.Log("GridMgr.RemoveXSTileByWorldPos: the tile is not exist, tilePos：" + tilePos);
                return false;
            }
        }

        public virtual bool HasXSTile(Vector3Int tilePos) => this.TileDict.ContainsKey(tilePos);

        public virtual bool HasXSTileByWorldPos(Vector3 worldPos) => this.TryGetXSTileByWorldPos(worldPos, out var tile);

        public virtual XSTile GetXSTile(Vector3Int tilePos)
        {
            this.TryGetXSTile(tilePos, out var tile);
            return tile;
        }

        public virtual XSTile GetXSTileByWorldPos(Vector3 worldPos)
        {
            this.TryGetXSTileByWorldPos(worldPos, out var tile);
            return tile;
        }

        public virtual bool TryGetXSTile(Vector3Int tilePos, out XSTile tile)
        {
        ...
        public virtual bool TryGetXSTileByWorldPos(Vector3 worldPos, out XSTile tile)
        {
            var tilePos = this.WorldToTile(worldPos);
            return this.TryGetXSTile(tilePos, out tile);
        }
